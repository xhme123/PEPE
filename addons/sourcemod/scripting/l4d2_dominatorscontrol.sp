...
#define DEBUG                0
#define NUM_SI_CLASSES        10
#define DOMINATORS_DEFAULT    1023  // 2^10 - 1, que permite todas las 10 clases como dominadores
...
public void OnPluginStart()
{
    ...
    g_hCvarDominators = CreateConVar(
        "l4d2_dominators",
        "1023",  // Valor por defecto para permitir todas las 10 clases como dominadores
        "Which infected class is considered as dominator (bitmask: 1 - smoker, 2 - boomer, 4 - hunter, 8 - spitter, 16 - jockey, 32 - charger, 64 - new_infected_1, 128 - new_infected_2, 256 - new_infected_3, 512 - new_infected_4)",
        _, true, 0.0, true, 1023.0
    );
...
void OnCvarDominatorsChange(ConVar hCvar, const char[] sOldVal, const char[] sNewVal)
{
    ...
    if (IsValidCvarValue(iNewVal)) {
        ...
        #if DEBUG
        PrintToServer("[DEBUG] l4d2_dominators changed to %d", g_iDominators);
        #endif
        SetDominators();
    } else {
        PrintToServer("[SM] Incorrect value of 'l4d2_dominators'! min: 0, max: %d", (1 << NUM_SI_CLASSES) - 1);
...
{
    return ((iValue >= 0) && (iValue <= 1023));  // 1023 es 2^10 - 1
}
